...
import {ipcRenderer} from "electron";
 function TableRow({search, fromDate, toDate}) {
  ...
  return (
    loading? <tr><td colSpan="7" className="w-full p-2 text-center text-2xl text-white font-semibold">Loading Data....</td></tr>
    :
    ( projects.length==0? <tr><td colSpan="7" className="w-full p-2 text-center text-2xl text-white font-semibold">No Record Found.</td></tr> :
    projects.map((project)=>{

        return (
          <tr className="border-t-2 border-gray-300 bg-blue-600 text-gray-100" key={project.id}>
            ...
            <td className="p-2">
              <button className="focus:outline-none " onClick={()=>{ clickHandler("delete", project.id)}} title="Delete Record">
            ...
            <td className="p-2">
              <button className="focus:outline-none "  title="View Details" onClick={()=>{ clickHandler("open", project.id)}}>
